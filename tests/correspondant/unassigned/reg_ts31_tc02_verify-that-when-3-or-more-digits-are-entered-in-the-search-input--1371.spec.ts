...
test.describe('Unassigned', () => {
  ...
  test('REG_TS31_TC02_Verify that when 3 or more digits are entered in the search input, bid map records are displayed in the popup, showing matching keywords within the categories of bid map name, column hea', async ({ page }) => {
    ...
    await page.locator("//select[@title=\"City\" or @title=\"Street\" or @title=\"State\"]/ancestor::div[@class=\"gap-2 header-grid-layout\"]//button[@aria-label=\"Edit Header\"]").click();
    await expect(page.locator("//h5[text()[normalize-space() = \"Update Header\"]]")).toBeVisible();
    await page.locator("//div[text()=\"Custom Header\"]/../..//input").clear();
    await page.locator("//div[text()=\"Custom Header\"]/../..//input").fill(testData["UniqueColumnHeaderSearch"]);
    await page.locator("//span[text()=\"Update Header\"]/../..//button").click();
    await page.locator("//span[contains(@class,'circle')]").waitFor({ state: 'hidden' });
