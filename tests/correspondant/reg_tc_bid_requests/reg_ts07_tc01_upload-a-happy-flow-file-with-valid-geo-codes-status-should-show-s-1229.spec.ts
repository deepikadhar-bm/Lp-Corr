...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  test('REG_TS07_TC01_Upload a happy flow file with valid geo codes; status should show success in both the popup and details.', async ({ page }) => {
    ...
    await page.locator("(//input[@type=\"file\"])[1]").setInputFiles(path.resolve(__dirname, 'test-data', "GEO_scenario_1.1_happy_flow.xlsx,GEO_scenario_1.1_happy_flow.xlsx"));
    vars["LoanNumberFromExcel"] = excelHelper.readCell(path.resolve(__dirname, 'test-data', "GEO_scenario_1.1_happy_flow.xlsx,GEO_scenario_1.1_happy_flow.xlsx"), "2", "1");
    ...
    await expect(page.locator("//div[text()=\" Geo Coding \"]/../..//span")).toContainText("Success");
    ...
    await expect(page.locator("(//td[@data-title=\"Errors\"]//div)[last()]")).toContainText(testData["Geo Coding happy flow (Errors Column)"]);
    await expect(page.locator("(//td[@data-title=\"Error Description\"]//div)[last()]")).toContainText(testData["Geo Coding happy flow(Error Description)"]);
    await expect(page.locator("(//td[@data-title=\"Loan Status\"]//div)[last()]")).toContainText(testData["Geo Coding happy flow(Loan Status)"]);
