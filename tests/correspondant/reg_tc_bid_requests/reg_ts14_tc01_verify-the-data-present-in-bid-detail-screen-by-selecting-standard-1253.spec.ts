...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  test('REG_TS14_TC01_Verify the data present in bid detail screen by selecting standard execution type', async ({ page }) => {
    ...
    vars["SelectedLockTerm"] = await page.locator("(//option[@value=\"3\"])[1]/../..//select[@aria-label=\"Dropdown selection\"]").evaluate(el => { const s = el as HTMLSelectElement; return s.options[s.selectedIndex]?.text || ''; });
    ...
    await expect(page.locator("//h5[contains(@aria-label,\"Execution Type\") and text()=\" SF($|SelectedLockTerm|d) \"]")).toBeVisible();
