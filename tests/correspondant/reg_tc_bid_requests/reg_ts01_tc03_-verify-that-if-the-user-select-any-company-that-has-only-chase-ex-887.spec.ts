...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  test('REG_TS01_TC03_ Verify that if the user select any company that has only chase execution type, then he should be able to select the valiues from the chase exe type only and standard should be disbaled.', async ({ page }) => {
    ...
    await expect(page.locator("//td[@data-title=\"Company Name\" and text()=\" @|CompanyName(CustomerPermissions)| \"]/ancestor::tr//td[@data-title=\"Chase Direct\"]//div[@class=\"text-success\"]")).toContainText("Allowed");
    ...
    await expect(page.locator("//label[text()[normalize-space() = \"Chase Direct\"]]/..//input")).toBeEnabled();
    await page.locator("//label[text()[normalize-space() = \"Chase Direct\"]]/..//input").check();
