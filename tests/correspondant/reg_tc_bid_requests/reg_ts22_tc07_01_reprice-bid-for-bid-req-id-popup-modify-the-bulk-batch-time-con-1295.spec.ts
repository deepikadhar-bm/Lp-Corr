...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  test('REG_TS22_TC07_01_Reprice bid for #Bid req ID\\\" popup : Modify the bulk batch time config for today, and validate that Today\\\'s pricing return time values are updated accordingly, when bid requested da', async ({ page }) => {
    ...
    while (parseFloat(String(vars["count"])) <= parseFloat(String(vars["TotalExtractedBatch"]))) {
      ...
      for (let dataIdx = parseInt(vars["count"]); dataIdx <= parseInt(vars["count"]); dataIdx++) {
        // Write to test data profile: "Stored_Text" = vars["EachBatchTiming"]
