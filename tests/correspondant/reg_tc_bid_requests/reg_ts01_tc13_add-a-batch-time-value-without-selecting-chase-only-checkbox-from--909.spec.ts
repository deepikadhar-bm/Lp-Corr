...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  test('REG_TS01_TC13_Add a batch time value (without selecting chase only checkbox) from the general settings module and verify that batch time value should be updated here under today\\\'s pricing return time', async ({ page }) => {
    // Prerequisite: REG_TS01_TC12_Delete batch time value from the general settings module and verify that batch time va
    ...
    vars["AddedBatchTime"] = await page.locator("(//div[@class=\"card-body\"]//*[text()=\"$|DeletedBatchTime1|\"])[last()]").textContent() || '';
    vars["PricingReturnTimeBuffer"] = await page.locator("//input[@id=\"pricingReturnTimeBuffer\"]").inputValue() || '';
    vars["AddedBatchTime"] = (() => {
