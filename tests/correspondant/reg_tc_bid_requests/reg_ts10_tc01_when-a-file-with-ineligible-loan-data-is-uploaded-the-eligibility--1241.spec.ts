...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  test('REG_TS10_TC01_When a file with ineligible loan data is uploaded, the eligibility pricing status should display as a warning, and the corresponding loans in the bid request details screen should be mar', async ({ page }) => {
    ...
    await expect(page.locator("//div[text()=\" Eligibility Pricing \"]/../..//span")).toContainText("Warning");
    ...
    if (String(testData["Eligibility Check(Error Column)"]).includes(String(vars["ErrorColumnData"]))) {
      await expect(page.locator("(//td[@data-title=\"Errors\"]//div)[last()]")).toContainText(testData["Eligibility Check(Error Column)"]);
    } else {
    ...
    await expect(page.locator("(//td[@data-title=\"Error Description\"]//div)[last()]")).toContainText(testData["Eligibility Check(Error Description)"]);
    await expect(page.locator("(//td[@data-title=\"Loan Status\"]//div)[last()]")).toContainText(testData["Eligibility Check(LoanStatus)"]);
