...
test.describe('Commitment List - TS_1', () => {
  ...
  test('REG_TS20_TC02_Verify the search action, Search by commitment ID, Bid request ID, Chase loan number and the Correspondent loan number for 3digit input)', async ({ page }) => {
    ...
    await page.locator("//td[@data-title=\"Comm. ID\"]//a[contains(@aria-label,\"View details for commitment\")]").waitFor({ state: 'visible' });
    ...
    while (parseFloat(String(vars["count"])) <= parseFloat(String(vars["CorrespondentLoanNumberCount"]))) {
      await page.locator("(//td[@data-title=\"Comm. ID\"]//a)[$|count|]").scrollIntoViewIfNeeded();
      vars["CommitID"] = await page.locator("(//td[@data-title=\"Comm. ID\"]//a)[$|count|]").textContent() || '';
      vars["CommitID"] = String(vars["CommitID"]).trim();
      await page.locator("(//td[@data-title=\"Comm. ID\"]//a)[$|count|]").click();
      await page.waitForLoadState('networkidle');
      await page.locator("//div[@aria-label=\"Sort by Loan Amount\"]").waitFor({ state: 'visible' });
      if (true) /* Element Req CarrLoan Num is visible */ {
      } else {
        expect((await page.locator("//td[@data-title=\"Corr. Loan#\"]//button[1]").textContent() || '').toLowerCase()).toContain(String('').toLowerCase());
      }
      await page.locator("//a[contains(text(),\"Commitment List\")]//i[contains(@class,\"fas fa-arrow-left \")]").click();
      await page.locator("//span[contains(@class,'circle')]").waitFor({ state: 'hidden' });
      vars["count"] = (parseFloat(String(vars["count"])) + parseFloat(String("1"))).toFixed(0);
