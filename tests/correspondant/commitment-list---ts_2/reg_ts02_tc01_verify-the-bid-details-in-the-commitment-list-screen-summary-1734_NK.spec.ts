...
import { BidRequestDetailsPage } from '@pages/correspondant';
...
test.describe('Commitment List - TS_2', () => {
  ...
  test('REG_TS02_TC01_Verify the Bid details in the commitment List screen summary', async ({ page }) => {
    ...
    // await page.locator("//div[contains(text(),'877O1E146138')]").click();
    await page.locator(
  "//div[contains(normalize-space(),'8702F2CA7FD8')]"
).first().waitFor({ state: 'visible' });

await page.locator("//a[contains(normalize-space(),'8702F2CA7FD8')]").first().click();
  await page.waitForTimeout(3000);
    ...
    vars["ExecutionTypePriceOfferedDetails"] = (await page.locator("//div[text()='Execution Type']//following-sibling::h5").textContent() || '').trim();
    console.log("ExecutionTypePriceOfferedDetails: " + vars["ExecutionTypePriceOfferedDetails"]);
    ...
    // await page.locator("//div[contains(@class, 'popover-body')]/ul[1]/li[3]/a[1]").click();
        await page.waitForTimeout(3000);
    // await page.locator("//td[@data-title='Bid Request ID']//div[normalize-space(text())=${vars["BidReqId"]}//ancestor::tr//td[@data-title='Comm. ID']//a[contains(@aria-label,'View details for commitme')]").click();
    await page.locator(
  `//td[@data-title='Bid Request ID']//div[contains(text(),'${vars["BidReqId"]}')]/ancestor::tr//td[@data-title='Comm. ID']//a[contains(@aria-label,'View details for commitme')]`).click();
    await page.waitForTimeout(3000);
