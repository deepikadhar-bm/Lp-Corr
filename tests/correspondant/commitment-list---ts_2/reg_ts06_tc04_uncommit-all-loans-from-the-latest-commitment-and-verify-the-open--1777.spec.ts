...
test.describe('Commitment List - TS_2', () => {
  ...
  test('REG_TS06_TC04_Uncommit all loans from the latest commitment and verify the open auth limit values + last commit values', async ({ page }) => {
    // Prerequisite: REG_TS06_TC03_Uncommit all loans from the commitment which is not latest commitment and verify the o
    // TODO: Ensure prerequisite test passes first

    await page.locator("//div[text()=\"Open Auth Limit: \"]//..//div[@class=\"fw-semibold\"]").waitFor({ state: 'visible' });
    vars["BidCommitTimeFirstLoan"] = await page.locator("//div[text()=\"Commit. Time\"]//following-sibling::h5").textContent() || '';
    vars["LastCommitLoanAmount"] = await page.locator("//td[@data-title=\"Loan Amount\"]").textContent() || '';
