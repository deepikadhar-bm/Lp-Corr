...
test.describe('REG_PriceOffered', () => {
  ...
  test('REG_TS17_TC01_Verify the download action and it should contain the all loans and locked loans information as present in the UI', async ({ page }) => {
    ...
    vars["AllLoansRowsCountUI"] = String(await page.locator("//tr[@role=\"row\"]").count());
    ...
    while (parseFloat(String(vars["count"])) <= parseFloat(String(vars["CountofHeaderNamesUI"]))) {
      vars["IndividualHeaderNameUIAllLoans"] = await page.locator("(//div[contains(@aria-label,\"Sort by\")])[$|count|]").textContent() || '';
    ...
    vars["TotalRowsCountUIAllLoans"] = String(await page.locator("//tbody//tr[@role=\"row\"]").count());
    ...
    while (parseFloat(String(vars["count"])) <= parseFloat(String(vars["TotalRowsCountUIAllLoans"]))) {
      ...
      vars["ColumnCountUIAllLoans"] = String(await page.locator("//table[@role='table']//tbody//tr[$|count|]//td[@data-title][position() >= 3]").count());
      vars["Count"] = "1";
      while (parseFloat(String(vars["Count"])) <= parseFloat(String(vars["ColumnCountUIAllLoans"]))) {
        vars["IndividualCellDataAllLoansUI"] = await page.locator("(//table[@role='table']//tbody//tr[$|count|]//td[@data-title][position() >= 3])[$|Count|]").textContent() || '';
