...
test.describe('REG_PriceOffered', () => {
  ...
  test('REG_TS27_TC02_Verify that once the commitment cut off time is passed the status with \\\"Partially Committed\\\" should get updated to \\\"expired\\\"', async ({ page }) => {
    ...
    vars["ClockFormat"] = String(vars["CurrentEstTime"]).slice(-2);
    ...
    await page.locator("//div[text()=\"Comm. Cut Off\"]/following::select[@class=\"form-select\"]").selectOption({ label: vars["ClockFormat"] });
    ...
    await page.locator("//div//input[@placeholder=\"Search By Bid Request ID\"]").fill(vars["BidReqIdPriceOffered"]);
    await page.locator("//span[contains(@class,'circle')]").waitFor({ state: 'hidden' });
    while (!(await page.locator("//a[contains(text(),\"$|BidReqIdPriceOffered|\")]//ancestor::tr//td[@data-title=\"Status\"]//span[text()[normalize-space() = \"Committed\"]]").isVisible())) {
      await page.reload();
      await page.locator("//span[contains(@class,'circle')]").waitFor({ state: 'hidden' });
      await page.locator("//div//input[@placeholder=\"Search By Bid Request ID\"]").fill(vars["BidReqIdPriceOffered"]);
      await page.locator("//span[contains(@class,'circle')]").waitFor({ state: 'hidden' });
    ...
    vars["BidStatusPriceOffered"] = await page.locator("//a[contains(text(),\"$|BidReqIdPriceOffered|\")]//ancestor::tr//td[@data-title=\"Status\"]//span").textContent() || '';
    vars["BidStatusPriceOffered"] = String(vars["BidStatusPriceOffered"]).trim();
    expect(String(vars["BidStatusPriceOffered"])).toBe("Committed");
    await page.locator("//span[text()[normalize-space() = \"Administration\"]]").click();
    await page.locator("//span[text()[normalize-space() = \"General Settings\"]]").click();
    await page.locator("//a[@href=\"#/admin/general-settings/early-close-config\"]").click();
