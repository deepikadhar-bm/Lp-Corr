...
test.describe('REG_General Settings', () => {
  ...
  test('REG_TS06_TC04_ Early config - try to add / update the record with the date which already existis and verify that system should not allow the duplicates', async ({ page }) => {
    ...
    await page.locator("//input[@id='lastBatchTime']").fill("11:59");
    ...
    await page.locator("(//td[contains(text(),\" $|TomorrowsDate|\")]/..//button)[2]").hover();
    await expect(page.getByText("Delete")).toBeVisible();
    await page.locator("(//td[contains(text(),\" $|TomorrowsDate|\")]/..//button)[2]").click();
    await page.locator("//button[@aria-label=\"Yes, Delete\"]").click();
    await page.locator("(//td[contains(text(),\" $|TomorrowsDate|\")]/..//button)[2]").waitFor({ state: 'hidden' });
